...
class Whatsiplus_WooCommerce_Frontend_Scripts implements Whatsiplus_Register_Interface {
	...
	public function msmswc_admin_enqueue_scripts() {
		wp_enqueue_script( 'admin-whatsiplus-scripts', plugins_url( 'js/admin.js?v=202012071500', __DIR__ ), array( 'jquery' ), '1.1.5', true );
		wp_enqueue_script( 'admin-whatsiplus-sendsms', plugins_url( 'js/sendsms.js', __DIR__ ), array(), false, true);
		wp_enqueue_script( 'admin-whatsiplus-charcounter', plugins_url( 'js/charactercounter.js', __DIR__ ), array(), false, true );
    ...
    public function load_bootstrap()
    {
        if ( isset($_GET['page']) ) {
            ...
            if ($pagenow === 'options-general.php' && $this->str_contains($page, 'whatsiplus-woocommerce-setting')) {
                wp_enqueue_style ( 'admin-whatsiplus-bootstrap', plugins_url( 'css/bootstrap.css', __DIR__));
                wp_enqueue_style ( 'admin-whatsiplus-wpfooter-fix', plugins_url( 'css/wpfooter-fix.css', __DIR__));
            }
        }
    }
